...
This document defines the **functional** and **non-functional** specifications for **Omnipresence**, a unified platform
for presence and attendance tracking across multiple domains.
...
Omnipresence aims to provide a single, adaptable system that enables organizations to record, manage, and analyze
presence data with minimal effort from daily users.
...
- The system shall allow domain-specific configuration of additional presence states (e.g., `late`, `excused`,
  `partial`, `checked_in`, `checked_out`).
...
Omnipresence is specified as a **domain-agnostic presence infrastructure**, designed to balance simplicity,
